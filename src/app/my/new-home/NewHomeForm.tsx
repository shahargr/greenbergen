    ...
    try {
      ...
      if (photo) {
        ...
        if (error) problems.push(`photo: ${error.message}`);
          ...
          // Into the property's album; the first photo becomes the cover.
          const r = await addPropertyPhoto(made.projectId, { path, name: photo.name || `photo${ext}`, mime: photo.type, size: photo.size });
