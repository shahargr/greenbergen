...
**Verified 2026-09-21**, all three running together: an edit to
`apps/contractor/src/app/layout.tsx` appeared at `localhost:3000/pro` within
seconds of saving. That is the test worth repeating if you ever suspect the
proxy — change a visible string in an app and look for it through port 3000.
Killing a dev server proves nothing, because `next dev` restarts its own
worker.
...
- **`next dev` writes agent files and keeps writing them.** It regenerates
  the "This is NOT the Next.js you know" block in the root CLAUDE.md, and the
  first time you run the two app servers it also creates
  `apps/homeowner/CLAUDE.md`, `apps/homeowner/AGENTS.md`,
  `apps/contractor/CLAUDE.md` and `apps/contractor/AGENTS.md` — four
  untracked files that appear out of nowhere in `git status`. They are the
  dev server's doing, not yours. Deleting them just brings them back on the
  next `npm run dev`.
