...
// portal must sit inside each project"). Every write goes through the gated
// RPCs (portal_budget_*, portal_bid_package_save); these actions only shape
// form data and route back.
...
const back = (projectId: string, q: string | null, extra: string) => {
  ...
  return `/project/${projectId}/finance?${extra}${qq}`;
};
...
// Open a bid package from a line - the bid room takes it from there.
