...
function LoginInner() {
  ...
  const next = rawNext && rawNext.startsWith("/") && !rawNext.startsWith("//") ? rawNext : "/work";
    ...
    <Screen>
      ...
      <form className="body" onSubmit={step === "email" ? send : verify} noValidate>
        ...
        <div className="actions" style={{ padding: 0, marginTop: "auto" }}>
          ...
          <p className="small text-muted center" style={{ margin: "4px 0 0" }}>New here? <Link href="/join">Join as a contractor</Link></p>
        </div>
      </form>
    </Screen>
  );
