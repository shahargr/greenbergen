...
| `portal_project_cancel` (migration 070) | **The other ending.** `Closed - Cancelled` joins the statuses: the work will not happen, which is a different fact from being left unfinished. Needs a reason, rank 50 and up, and it cancels the open work in the family through `close_action` — each task carrying that reason. Money already paid is reported, never a blocker. `Closed - Incomplete` stays superadmin-only. |
| `portal_project_close` / `portal_project_reopen` (migration 069) | **Finish a job from inside it.** Rank 50 and up, or superadmin. The old rules stand: zero open tasks (a hard rule with no override), and closing FREEZES the project's tasks, contracts and transactions. New: open tasks are counted across the whole family, and a live child job refuses the close on its own — a row trigger cannot see either. Money is reported, never a blocker. Reopening is superadmin only. ⚠️ `sgr.superadmin = on` grants superadmin to **everything** for the rest of the transaction, so the reopen raises it around one UPDATE and restores the previous value, error path included. |
| `portal_site_visit_log` / `_edit` / `_delete`, `portal_site_visits`, `portal_site_week` (migration 068) | **A site visit is one record**, not an arrive/leave pair: the day, what you saw, and whatever you attached (photo, file, voice note), correctable and removable by whoever wrote it or whoever runs the site. It also puts you on that day's roster. `portal_site_week` answers **who is on site this week, by trade** — a trade counts if someone of that trade is on the roster in the week OR there is open work in that trade dated inside it; each panel leads to `/project/[id]/trade/[trade]`, that trade's people, money and work. Money in both is gated by `can_view_project_financials`. |
| `portal_tasks` (migration 066) | Every task carries its `trade` (from its contract, else its scope line, else its person), its `contract` (id + title), and its `phase` — `trades.stage`, ordered by `trade_stages.sort_order`. Coverage on 55 Walnut is thin (trade 25%, phase 24%, contract 10%), so the project screen keeps **timing** as the default grouping and offers the other three as a switch, each naming what it cannot place ("No trade recorded · 98"). `p_closed_limit` brings finished work back; `getBoard({ closed })` asks for it only where Done is offered. |
| `task_payment_log` / `portal_task_detail` (migration 065) | What a task COST, with no contract behind it — a sign bought online, a part, a permit fee. Writes a transaction with `action_id` set and `contract_id` null, so it lands under the money page's other costs. The payee is required (the table refuses a priced row with neither a contract nor a contractor); a name that matches nothing on file becomes a contact. A purchase logged with the receipt in hand opens no "Awaiting confirmation" task. |
