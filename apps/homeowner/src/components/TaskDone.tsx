...
// Most of what a homeowner has to say about an open task is the middle of
// the story - I called the town, here are the photos the inspector wanted,
// still waiting on a part - and closing the task used to be the price of
// being heard. So the sheet posts an ENTRY: a comment, as many photos or
// files as they like, and then either
...
// record_project_file, and go to homeowner_task_update together.
...
export function TaskDone({ projectId, actionId, title, onDone }: { projectId: string; actionId: string; title: string; onDone?: () => void }) {
  ...
  return (
    <>
      <button type="button" className="btn btn-secondary" style={{ minHeight: 40 }} onClick={() => setOpen(true)}>Update</button>
      {open && (
        <div className="sheet-back" role="dialog" aria-modal="true" aria-label="Update this task" onClick={(e) => { if (e.target === e.currentTarget && !busy) cancel(); }}>
          <div className="sheet">
            <div className="sheet-head"><div className="title">Update, or flag as complete</div></div>
