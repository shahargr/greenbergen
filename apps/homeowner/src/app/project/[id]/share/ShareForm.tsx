    ...
    <form action={publishShare} className="stack" onSubmit={() => setBusy(true)}>
      ...
      <div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8 }}>
          {photos.map((p) => (
            <button type="button" key={p.id} onClick={() => setAfter(p.id === after ? "" : p.id)} className="blueprint" style={{ padding: 0, aspectRatio: "1", background: "var(--color-neutral-200)", position: "relative", cursor: "pointer", outline: after === p.id ? "2px solid var(--color-accent)" : "none" }} aria-pressed={after === p.id}>
              {p.url && (
          ...
          <Link href={`/project/${projectId}/timeline`} className="blueprint" style={{ aspectRatio: "1", display: "grid", placeItems: "center", textDecoration: "none", fontSize: 12, textAlign: "center", padding: 6 }}>+ add<br />after photo</Link>
        </div>
      </div>
      ...
      <div className="actions" style={{ padding: 0 }}>
        <button className={`btn btn-primary btn-block blueprint ${busy ? "busy" : ""}`} disabled={busy}>{busy ? <><span className="spin" /> Building your card…</> : "Share with the neighbors"}</button>
