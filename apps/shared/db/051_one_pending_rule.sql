-- 051 - one pending rule, and the counter follows the repo.
--
-- The repair after auditing 050 (the chat session's v219). Its rewrite of
-- portal_my_messages kept 043's mine / self / Me and added two good things
-- (a created_by of system:% is a system message; a system message has no
-- counterparty to reply to) but changed the PENDING test to
--   read_at is null and status = 'new'
-- while my_unread_count() and portal_my_messages_pending() - the badge on
-- every shell - still say
--   read_at is null and handled_at is null and status not in ('dismissed','done').
-- Two tests for one dot is how a badge says 1 above an empty list. One
-- rule again, the badge's. It also dropped STABLE from a read; restored.
--
-- The counter: v219 set config.schema_version = 219 by hand, under its own
-- numbering; this repo's files had reached 263, and 049 made it 264. Shahar
-- (2026-09-10): use the higher number. Set to 264 in the same session; this
-- migration bumps it to 265 like every other.
...
as $function$
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'kind', case
      ...
      -- Written by the platform, not by a person: no counterparty to reply to (v219).
      when x.created_by like 'system:%' then 'system'
    ...
    'with_contact_id', case when x.created_by like 'system:%' then null
                            when x.from_contact_id = (select contact_id from me) then x.to_contact_id
                            else x.from_contact_id end,
